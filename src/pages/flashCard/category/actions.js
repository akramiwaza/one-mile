...
export const GetFlashCardCategory =
  ({ callback }) =>
  async (dispatch, getState) => {
    try {
      let link = UrlApi.flashcard.category.get;
...
export const ManageFlashCardCategory =
  ...
  async (dispatch, getState) => {
    try {
      ...
      const response = await PostFunction({
        data: bodyFormData,
        Route: editMode
          ? UrlApi.flashcard.category.edit
          : UrlApi.flashcard.category.add,
        showalert: true,
        state: getState,
        isMultipart: true,
        dispatch: dispatch,
...
export const EditFlashCardCategory =
  ...
  async (dispatch, getState) => {
    try {
      ...
      const response = await PostFunction({
        data,
        Route: UrlApi.flashcard.category.edit,
        showalert: true,
        state: getState,
        isMultipart: false,
        dispatch: dispatch,
...
export const DeleteFlashCardCategory =
  ({ callback, id }) =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      ...
      const response = await PostFunction({
        data,
        Route: UrlApi.flashcard.category.delete,
        showalert: true,
        state: getState,
        isMultipart: false,
        dispatch: dispatch,
